...
def fetch_cost_for_subscription(token, subscription_id, start_date, end_date):
    ...
    Returns: (cost_data dict, status_code int, reason str)
    """
    try:
        ...
        if status_code != 200:
            # Try to extract a meaningful error message from Azure API response
            try:
            ...
            logger.warning(f"Cost fetch failed for {subscription_id} | Status: {status_code} | Reason: {error_detail}")
            return {"properties": {"rows": [], "columns": []}}, status_code, error_detail

        cost_data = response.json()
        rows = cost_data.get("properties", {}).get("rows", [])
        logger.info(f"  Cost data retrieved: {len(rows)} rows | Status: {status_code}")

        # Even on 200, rows can be empty (subscription had zero spend in range)
        if not rows:
            reason = "API call successful but no cost data found for this period (zero spend or no resources)"
        else:
            reason = "Success"

        return cost_data, status_code, reason

    except requests.exceptions.Timeout as e:
        logger.warning(f"Timeout fetching cost for {subscription_id}: {str(e)}")
        return {"properties": {"rows": [], "columns": []}}, "TIMEOUT", f"Request timed out: {str(e)}"
    except Exception as e:
        logger.warning(f"Error fetching cost for {subscription_id}: {str(e)}")
        return {"properties": {"rows": [], "columns": []}}, "EXCEPTION", f"Unexpected error: {str(e)}"

def generate_csv(all_costs_data, start_date_display, end_date_display):
    """Generate CSV with all subscriptions cost data including API status and reason"""
    try:
        ...
        # Write headers — now includes API Status Code and Reason/Notes
        csv_writer.writerow([
            "Subscription Name",
            "Subscription ID",
        ...
        for cost_item in all_costs_data:
            ...
            status_code = cost_item["api_status_code"]
            reason      = cost_item["reason"]
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <p style="color: #555; font-size: 0.95em;">
                    The CSV includes an <strong>API Status Code</strong> and <strong>Reason / Notes</strong> column
                    for each subscription so you can quickly identify any data fetch issues.
                </p>
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        for idx, subscription in enumerate(subscriptions, 1):
            ...
            # Now unpacking 3 values: cost_data, status_code, reason
            cost_data, api_status_code, reason = fetch_cost_for_subscription(
                token, sub_id, start_date_api, end_date_api
            )
